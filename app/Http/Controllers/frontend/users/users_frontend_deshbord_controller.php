...
class users_frontend_deshbord_controller extends Controller
{
    ...
    public function users_home_controller(Request $request, $id=null)
    {
        ...
        if(in_array(0, $accessCategory)){
            $cat = Category::orderBy('role', 'ASC')->where('st', 1)->get();
        }else{
            $cat = Category::orderBy('role', 'ASC')->where('st', 1)->whereIn('id', $accessCategory)->get();
        }
        ...
        return view('users.pages.home.home') -> with(compact('products', 'slider', 'userData', 'where', 'management', 'cat', 'id', 'cat_f', 'cat_r', 'creator', 'packages'));
